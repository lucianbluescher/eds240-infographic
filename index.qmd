...
Below are two component visuals rendered directly in this post (code adapted from my `final_graphics/` QMDs). When I move this file into my website repo, it should still run as long as the same `data/` paths are available from the site root.
...
Other key panels (renderable from QMDs in `final_graphics/`) cover:
...
I add just enough context to avoid misinterpretation (e.g., noting Soued is all reservoirs; explaining that methane drives rising radiative forcing even as emissions decline).
...
## What this infographic argues (in one sentence)
